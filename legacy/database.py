...
def init_db():
    ...
    c.execute("""
        CREATE TABLE IF NOT EXISTS heat_scores (
            ...
            install_velocity REAL DEFAULT 0,
            rating_velocity REAL DEFAULT 0,
            chart_momentum REAL DEFAULT 0,
            region_momentum REAL DEFAULT 0,
            newness_bonus REAL DEFAULT 0,
            UNIQUE(app_id, date)
        )
    """)
    ...
    c.execute("""
        CREATE TABLE IF NOT EXISTS alerts_log (
            ...
            app_id TEXT,
            UNIQUE(alert_key, timestamp)
        )
    """)

    c.execute("CREATE INDEX IF NOT EXISTS idx_heat_scores_date ON heat_scores(date)")
    c.execute("CREATE INDEX IF NOT EXISTS idx_heat_scores_app ON heat_scores(app_id, date)")
    c.execute("CREATE INDEX IF NOT EXISTS idx_alerts_key_date ON alerts_log(alert_key, timestamp)")
...
def save_heat_score(data: dict, date: str):
    """Save or update a heat score record."""
    conn = get_conn()
    conn.execute("""
        INSERT OR REPLACE INTO heat_scores
            (app_id, date, heat_score, install_velocity, rating_velocity,
             chart_momentum, region_momentum, newness_bonus)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data["app_id"], date, data["heat_score"],
        data["install_velocity"], data["rating_velocity"],
        data["chart_momentum"], data["region_momentum"],
        data["newness_bonus"],
    ))
    conn.commit()
    conn.close()
...
def get_heat_scores(date: str | None = None):
    """Get heat scores for a given date (default: latest)."""
    conn = get_conn()
    if not date:
        row = conn.execute("SELECT MAX(date) as d FROM heat_scores").fetchone()
        date = row["d"] if row and row["d"] else datetime.utcnow().strftime("%Y-%m-%d")
    rows = conn.execute("""
        SELECT hs.*, a.title, a.developer, a.genre, a.first_seen_date
        FROM heat_scores hs
...
def get_app_heat_history(app_id: str):
    """Get heat score history for an app."""
    conn = get_conn()
    rows = conn.execute("""
        SELECT * FROM heat_scores WHERE app_id = ? ORDER BY date ASC
    """, (app_id,)).fetchall()
    conn.close()
    return [dict(r) for r in rows]


def get_top_heated(limit: int = 20, date: str | None = None):
    """Get top apps by heat score."""
    conn = get_conn()
    if not date:
        row = conn.execute("SELECT MAX(date) as d FROM heat_scores").fetchone()
        date = row["d"] if row and row["d"] else datetime.utcnow().strftime("%Y-%m-%d")
    rows = conn.execute("""
        SELECT hs.*, a.title, a.developer, a.genre, a.icon_url, a.first_seen_date
        FROM heat_scores hs
        JOIN apps a ON hs.app_id = a.app_id
        WHERE hs.date = ?
        ORDER BY hs.heat_score DESC
...
def save_alert_log(alert_key: str, alert_type: str, app_id: str):
    """Log an alert to prevent duplicates."""
    conn = get_conn()
    today = datetime.utcnow().strftime("%Y-%m-%d")
    conn.execute("""
        INSERT OR IGNORE INTO alerts_log (timestamp, alert_key, alert_type, app_id)
        VALUES (?, ?, ?, ?)
    """, (today, alert_key, alert_type, app_id))
    conn.commit()
    conn.close()


def was_alert_sent_today(alert_key: str) -> bool:
    """Check if this alert was already sent today."""
    conn = get_conn()
    today = datetime.utcnow().strftime("%Y-%m-%d")
    row = conn.execute("""
        SELECT 1 FROM alerts_log WHERE alert_key = ? AND timestamp = ?
    """, (alert_key, today)).fetchone()
    conn.close()
...
def get_category_growth_stats(days: int = 7):
    """Aggregate install growth by genre over the last N days."""
    conn = get_conn()
    cutoff = (datetime.utcnow() - timedelta(days=days)).strftime("%Y-%m-%d")
    rows = conn.execute("""
        SELECT
            ...
            SUM(s2.real_installs - s1.real_installs) AS total_growth,
            AVG(s2.real_installs - s1.real_installs) AS avg_growth
        FROM apps a
        JOIN snapshots s1 ON a.app_id = s1.app_id AND s1.region = 'us'
        JOIN snapshots s2 ON a.app_id = s2.app_id AND s2.region = 'us'
        WHERE s1.date = (
            SELECT MIN(date) FROM snapshots WHERE app_id = a.app_id AND region = 'us' AND date >= ?
        )
        AND s2.date = (
            SELECT MAX(date) FROM snapshots WHERE app_id = a.app_id AND region = 'us'
        )
        AND s2.real_installs > s1.real_installs
...
def get_region_growth_stats(days: int = 7):
    """Aggregate install growth by region over the last N days."""
    conn = get_conn()
    cutoff = (datetime.utcnow() - timedelta(days=days)).strftime("%Y-%m-%d")
    rows = conn.execute("""
        SELECT
            s2.region,
            COUNT(DISTINCT s2.app_id) AS app_count,
            SUM(s2.real_installs - s1.real_installs) AS total_growth,
            AVG(s2.real_installs - s1.real_installs) AS avg_growth
        FROM snapshots s1
        JOIN snapshots s2 ON s1.app_id = s2.app_id AND s1.region = s2.region
        WHERE s1.date = (
            SELECT MIN(date) FROM snapshots WHERE app_id = s1.app_id AND region = s1.region AND date >= ?
        )
        AND s2.date = (
            SELECT MAX(date) FROM snapshots WHERE app_id = s2.app_id AND region = s2.region
        )
        AND s2.real_installs > s1.real_installs
        GROUP BY s2.region
        ORDER BY total_growth DESC
...
def get_top_movers(region: str = "us", days: int = 7):
    """Apps with biggest chart position improvement over the last N days."""
    conn = get_conn()
    cutoff = (datetime.utcnow() - timedelta(days=days)).strftime("%Y-%m-%d")
    rows = conn.execute("""
        SELECT
            ...
            a.title,
            a.genre,
            cp_old.position AS old_position,
            cp_new.position AS new_position,
            (cp_old.position - cp_new.position) AS improvement,
            cp_new.chart_type,
            cp_new.category
        FROM chart_positions cp_old
        JOIN chart_positions cp_new ON cp_old.app_id = cp_new.app_id
            AND cp_old.region = cp_new.region AND cp_old.chart_type = cp_new.chart_type
        JOIN apps a ON cp_old.app_id = a.app_id
        WHERE cp_old.region = ?
        AND cp_old.date = (
            SELECT MIN(date) FROM chart_positions
            WHERE app_id = cp_old.app_id AND region = ? AND date >= ?
        )
        AND cp_new.date = (
            SELECT MAX(date) FROM chart_positions
            WHERE app_id = cp_new.app_id AND region = ?
        )
        AND cp_old.position > cp_new.position
        ORDER BY improvement DESC
        LIMIT 50
    """, (region, region, cutoff, region)).fetchall()
    conn.close()
    return [dict(r) for r in rows]


def get_weekly_summary():
    """Summary stats for the past week."""
    conn = get_conn()
    week_ago = (datetime.utcnow() - timedelta(days=7)).strftime("%Y-%m-%d")
    today = datetime.utcnow().strftime("%Y-%m-%d")

    new_apps = conn.execute(
        "SELECT COUNT(*) as cnt FROM apps WHERE first_seen_date >= ?", (week_ago,)
    ).fetchone()["cnt"]

    total_active = conn.execute(
        "SELECT COUNT(*) as cnt FROM apps WHERE status = 'active'"
    ).fetchone()["cnt"]

    # Top heat score this week
    top_heat = conn.execute("""
        SELECT hs.app_id, a.title, MAX(hs.heat_score) as peak_score
        FROM heat_scores hs
        JOIN apps a ON hs.app_id = a.app_id
        WHERE hs.date >= ?
        GROUP BY hs.app_id
    ...
    return {
        ...
        "top_heat": [dict(r) for r in top_heat],
