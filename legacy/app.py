...
from database import (
    ...
    get_top_heated, get_app_heat_history, get_heat_scores,
    get_category_growth_stats, get_region_growth_stats, get_top_movers,
    get_weekly_summary,
)
...
# --- Tabs ---
tab_emerging, tab_prereg, tab_all, tab_detail, tab_trends, tab_heatmap, tab_movers, tab_quality = st.tabs(
    ["Emerging Hits", "Pre-Registrations", "All Apps", "App Detail",
     "Trend Analytics", "Category Heatmap", "Top Movers", "Data Quality"]
)
...
with tab_emerging:
    ...
    if em_df.empty:
        ...
        # Merge heat scores
        heat_data = get_heat_scores()
        if heat_data:
            heat_df = pd.DataFrame(heat_data)[["app_id", "heat_score"]].rename(columns={"heat_score": "heat"})
            em_df = em_df.merge(heat_df, on="app_id", how="left")
        else:
            em_df["heat"] = None
        # Metrics row
        new_count = len(em_df)
        ...
        if "Heat" in display_df.columns:
            display_df["Heat"] = display_df["Heat"].apply(
...
with tab_prereg:
    ...
    st.caption("Apps in pre-registration status, discovered via developer pages.")

    prereg_apps = get_pre_registrations()
    if not prereg_apps:
        st.info("No pre-registration apps found yet. Run `python discovery.py` — it scans all known developers' pages.")
    else:
        pr_df = pd.DataFrame(prereg_apps)
        pr_df["store_url"] = "https://play.google.com/store/apps/details?id=" + pr_df["app_id"]

        # Sort by first_seen_date descending (newest first)
        if "first_seen_date" in pr_df.columns:
            pr_df = pr_df.sort_values("first_seen_date", ascending=False)

        st.metric("Total Pre-Registrations", len(pr_df))
...
with tab_detail:
    ...
    if detail_df.empty:
        ...
        if not app_options:
            ...
            heat_hist = get_app_heat_history(selected_app_id)
            if heat_hist:
                latest_heat = heat_hist[-1]
                hcol1, hcol2, hcol3, hcol4, hcol5, hcol6 = st.columns(6)
                hcol1.metric("Total", f"{latest_heat['heat_score']:.0f}/100")
                hcol2.metric("Install Vel.", f"{latest_heat['install_velocity']:.0f}")
                hcol3.metric("Rating Vel.", f"{latest_heat['rating_velocity']:.0f}")
                hcol4.metric("Chart Mom.", f"{latest_heat['chart_momentum']:.0f}")
                hcol5.metric("Region Mom.", f"{latest_heat['region_momentum']:.0f}")
                hcol6.metric("Newness", f"{latest_heat['newness_bonus']:.0f}")

                if len(heat_hist) >= 2:
                    heat_df = pd.DataFrame(heat_hist)
                    fig = px.line(heat_df, x="date", y="heat_score", title="Heat Score Over Time")
                    fig.update_yaxes(range=[0, 100])
                    st.plotly_chart(fig, use_container_width=True)
            else:
                st.info("No heat score data yet. Run tracker.py to compute scores.")
...
with tab_trends:
    ...
    col1.metric("New Apps This Week", summary["new_apps"])
    col2.metric("Total Active Apps", summary["total_active"])
    col3.metric("Period", f"{summary['period_start']} to {summary['period_end']}")

    # Top 10 by heat score
    st.markdown("### Top 10 by Heat Score")
    top_heat = get_top_heated(limit=10)
    if top_heat:
        th_df = pd.DataFrame(top_heat)
        th_df["store_url"] = "https://play.google.com/store/apps/details?id=" + th_df["app_id"]
        display_cols = ["title", "store_url", "heat_score", "genre", "developer",
                        "install_velocity", "rating_velocity", "chart_momentum",
                        "region_momentum", "newness_bonus"]
        available = [c for c in display_cols if c in th_df.columns]
        st.dataframe(
            th_df[available].rename(columns={
                "title": "App", "store_url": "Store", "heat_score": "Heat",
                "genre": "Genre", "developer": "Developer",
                "install_velocity": "Install Vel", "rating_velocity": "Rating Vel",
                "chart_momentum": "Chart Mom", "region_momentum": "Region Mom",
                "newness_bonus": "Newness",
            }),
            use_container_width=True,
            column_config={"Store": st.column_config.LinkColumn(display_text="Open")},
        )
    else:
        st.info("No heat score data yet.")

    # Region growth
    st.markdown("### Growth by Region (last 7 days)")
    region_stats = get_region_growth_stats(7)
    if region_stats:
        rg_df = pd.DataFrame(region_stats)
        fig = px.bar(rg_df, x="region", y="total_growth", title="Total Install Growth by Region",
                     text="app_count", labels={"total_growth": "Total Growth", "region": "Region"})
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.info("Not enough data for region growth analysis.")

# ==================== TAB 6: Category Heatmap ====================
with tab_heatmap:
    st.title("Category Heatmap")

    heatmap_days = st.selectbox("Period", [7, 14, 30], index=0, format_func=lambda x: f"{x} days", key="heatmap_days")
    cat_stats = get_category_growth_stats(heatmap_days)

    if cat_stats:
        cat_df = pd.DataFrame(cat_stats)
        cat_df["avg_growth"] = cat_df["avg_growth"].fillna(0).astype(int)
        cat_df = cat_df.sort_values("total_growth", ascending=False)

        fig = px.bar(cat_df, x="genre", y="total_growth", color="avg_growth",
                     title="Install Growth by Genre",
                     labels={"genre": "Genre", "total_growth": "Total Growth", "avg_growth": "Avg Growth"},
                     text="app_count")
        ...
        st.dataframe(
            cat_df.rename(columns={
                "genre": "Genre", "app_count": "Apps",
                "total_growth": "Total Growth", "avg_growth": "Avg Growth/App",
            }),
            use_container_width=True,
        )
    else:
...
with tab_movers:
    st.title("Top Movers")
    st.caption("Apps with the biggest chart position improvements.")

    movers_region = st.selectbox("Region", available_regions, key="movers_region")
    movers_days = st.selectbox("Period", [7, 14, 30], index=0, format_func=lambda x: f"{x} days", key="movers_days")

    movers = get_top_movers(movers_region, movers_days)
    if movers:
        mv_df = pd.DataFrame(movers)
        mv_df["store_url"] = "https://play.google.com/store/apps/details?id=" + mv_df["app_id"]
        mv_df["change"] = mv_df.apply(lambda r: f"#{r['old_position']} -> #{r['new_position']} (+{r['improvement']})", axis=1)

        st.dataframe(
            mv_df[["title", "store_url", "genre", "chart_type", "change", "improvement"]].rename(columns={
                "title": "App", "store_url": "Store", "genre": "Genre",
                "chart_type": "Chart", "change": "Position Change", "improvement": "Climb",
            }),
            use_container_width=True, height=600,
            column_config={"Store": st.column_config.LinkColumn(display_text="Open")},
        )
    else:
        st.info("No chart movement data for this period.")
